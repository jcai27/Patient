...
- **Stress store**: Optional `stress_profile.json` generated by the Stress Analyzer (text/audio cues)
...
5. **Stress Analyzer**: (Ingestion-time) mines transcript + audio to capture stress hotspots and cadence hints that feed into the Contextor
...
## Stress Analyzer (optional but recommended)

- During ingestion you can supply `audio_dir` (CLI or API) pointing to `.wav` interviews. The Stress Analyzer agent scores text spans and acoustic spikes, saving `stress_profile.json`.
- `stress_profile.json` feeds the Contextor so style prompts capture how the real voice hedges, pauses, or sharpens when stressed.
- Audio analysis requires `praat-parselmouth` (installed via `pip install praat-parselmouth`). If the library is missing the analyzer gracefully degrades to text-only cues.
- Outputs include hotspot quotes, acoustic metrics (pitch/jitter/intensity deltas), and a short narrative hint the style pack can lean on when recreating the persona's cadence.
